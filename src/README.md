# Vision Tactile Sensor Design Platform

With FEM + iFEM + Synthetic Camera data generation + AI model and training

- **Forward FEM**: given a dense surface force distribution (Fx, Fy, Fz), solve 3D deformation of a silicone block (supports multi layers with different physical properties).
- **Inverse FEM (iFEM)**: given (measured or simulated) multi-depth displacement, recover the surface 3D force field applied (with regularization/constraints).
- **End-to-end tests**: generate test forces → forward simulate → sample/“observe” → inverse solve → verify accuracy.
- **Synthetic data generation**: rendering synthetic sensor camera images, generation of trajectories, and full datasets of variety of different types of vision tactile sensor designs.  plug-in mechanism, allow you to add your own sensor design.
- **AI Model** : provide reference model design (CNN with PINN loss) and complete traning pipeline.
...
- Visualization:
  ...
  - TODO: 3D mesh deformation, slicing, and interactive views (PyVista/VTK).
- Synthetic **vision tactile sensor renderer**, exports camera frames and (optionally) ground-truth deformation image.
- **AI Model** : provide reference model design (CNN with PINN loss) and complete traning pipeline.
- **TODO:** End-to-end regression tests (RMSE, correlation, angle error, SSIM/PSNR for `tz`).

---

## 2) Environment and OS notes

- **FEM and iFEM are using dolfinx, is best on Linux**. For Windows, use **WSL2 (Ubuntu)**.  
...
### Generate a synthetic surface 3D force field (this is sensor agnostic):
...
### Forward FEM (3D surface force field → 3D Displacements of gel mesh):
...
### Inverse (displacement/observations → 3D surface force field):
...
```bash
python src/scripts/render_camera_frame.py   --config src/config/renderer_particles.yaml   --out data/output/render.png   --seed 42   --supersample 2
```
...
2. **modify** the sensor config file `renderer_xxx.yaml` (`renderer_particles.yaml`, `renderer_tac3d.yaml`, `renderer_gelsight.yaml`):
...
### Generate Dataset for AI Model Training

Generate a single sample as test:
...
**Generate Dataset (many random samples, in parallel)**:
...
Details of model traing and test see: [src/nn/README.md](src/nn/README.md)

---

### End-to-end validation (**TODO: Not Implemented Yet**):
